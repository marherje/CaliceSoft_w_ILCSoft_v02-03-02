#ifndef _HistogramCollection_t_hh_
#define _HistogramCollection_t_hh_
...
namespace histmgr {
   ...
   */
  class HistogramCollection_t : public reference_counter {
  ...
  public:
     ...
     */
    HistogramCollection_t(const std::string &collection_name,
    ...
    {
      createHistograms(collection_name, name_list, n_hist, par);
      //    __instances++;
    }
    ...
    HistogramCollection_t(const std::string &collection_name,
      ...
      : _histogramCol(0),
        _majorIndex(0)
    {
      createHistograms(collection_name,name_list, n_hist_list, par);
      //    __instances++;
    }
     ...
     */
    HistogramCollection_t(lcio::LCCollection *histograms, lcio::IntVec *indices)
     ...
     * @param histograms the linearised one or two dimensional collection of histograms (two dimensional collections have the collection parameter "major").
     ...
     */
    HistogramCollection_t(lcio::LCCollection *histograms)
      : _histogramCol(histograms),
        _majorIndex(0)
    {
      //    stat_copy(_histogramCol);
    ...
    /** copy constructor.
     */
    HistogramCollection_t(const HistogramCollection_t &a)
    ...
    /** Default constructor.
     */
    HistogramCollection_t() : _histogramCol(0),_majorIndex(0) {};
     ...
     */
    const FloatHistogram1D *histogram(UInt_t index) const {
#ifdef BOUNDARY_CHECK
...
#endif
      return dynamic_cast<FloatHistogram1D*>(_histogramCol->getElementAt(index));
    };

    /** Get one histogram of the  collection.
     */
    FloatHistogram1D *histogram(UInt_t index)  {
#ifdef BOUNDARY_CHECK
      if (index>=static_cast<UInt_t>(_histogramCol->getNumberOfElements())) {
        std::stringstream message;
        message << "Range error accessing histogram " << index << ". Ther are only "
     ...
     */
    FloatHistogram1D *histogram(UInt_t major_index, UInt_t minor_index)  {
#ifdef BOUNDARY_CHECK
      if (!_majorIndex) {
     ...
     */
    const FloatHistogram1D *histogram(UInt_t major_index, UInt_t minor_index) const {
#ifdef BOUNDARY_CHECK
      if (!_majorIndex) {
        throw std::logic_error("A major index was specified but the index collection does not exist.");
      }
